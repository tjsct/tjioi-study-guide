...
Graph theory is heavy in vocabs, but we will just go over the most important ones.
...
% this section is kind of messy bc im sleep deprived sorry
% it turns out that this is more like talking about trees. it might be more useful to actually
% introduce it that way first, then show how it applies to graphs as well.
% kevin, 2017-02-24
...
It turns out that the only difference between a \textit{graph} and a \textit{tree} is that a tree is a graph without cycles. So if we want to extend our knowledge of tree traversal to graph traversal, all we have to do is avoid cycles.
